...
class Journeys_Archive_Sign_Up_Form extends Widget_Base
{

    public function get_name()
    {
        return 'journeys_archive_sign_up_form';
    }

    public function get_title()
    {
        return __('Journeys Archive Sign Up Form', 'paramedics-theme');
    }
    ...
    protected function render()
    {
        $settings = $this->get_settings_for_display();
        $widget_id = $this->get_id();
        $journeys_data = $this->get_journeys_data();
        $journeys_ids = array_column($journeys_data, 'id');
        $form_title = $settings['title'];
        $form_description = $settings['description'];

        $sign_up_form_template = require CHILD_THEME_DIR . '/includes/elementor/templates/journeys-archive-sign-up-form.php';
    ...
    private function get_journeys_data()
    {
        $journeys_ids = get_posts([
            'post_type' => 'trip',
            'posts_per_page' => -1,
            'fields' => 'ids',
        ]);
        ...
        foreach ($journeys_ids as $journey_id) {
            $journeys_data[] = $this->get_journey_data($journey_id);
        }

        return $journeys_data;
    }

    private function get_journey_data($journey_id)
    {
        $title = get_the_title($journey_id);
        $location = get_field('location', $journey_id);
        $payment = get_field('payment', $journey_id);
        $dates = get_field('dates', $journey_id);
        return [
            'id' => $journey_id,
            'title' => $title,
            'location' => $location,
